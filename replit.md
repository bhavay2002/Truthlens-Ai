...
- **feature_schema.make_vector**: Changed `clip` default from `(0.0, 1.0)` to `None`; rewrote `strict=True` to collect all missing and unknown keys up-front and raise with messages matching "Missing required feature keys" / "Unknown feature keys".
- **_text_features.extract_alpha_lemmas**: Added `.isalpha()` guard on the lowercased lemma so non-alphabetic lemmas (e.g., morphological edge cases) are excluded.
- **narrative_role_extractor**: Added `hero_entities`, `villain_entities`, `victim_entities` (as float counts) to `_role_scores()` and `_empty()` output; updated `NARRATIVE_ROLE_KEYS` in `feature_keys.py`.
- **en_core_web_sm model**: Installed via `python -m spacy download en_core_web_sm` — required for all spaCy-dependent analyzers; fixes `TestNlpCache::test_different_disable_gives_different_instance`.
- **ExplanationAggregator.aggregate**: Added `shap_importance` / `attention_scores` keyword args (list-of-dicts format) with a short-circuit path that returns a plain dict with union tokens and normalized importances.
